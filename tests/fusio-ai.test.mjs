...
test("Fusio AI keeps NVIDIA credentials server-side and defaults to the fast tool-capable model", async () => {
  const edge = await read("supabase/functions/fusio-ai/index.ts");
  const client = await read("src/services/fusioAI.js");
...
test("Fusio AI exposes catalog, provider and Nucleus tools to the agent", async () => {
  const edge = await read("supabase/functions/fusio-ai/index.ts");
...
test("Fusio AI shell mirrors the Synapse launcher pattern without AI branding icons", async () => {
  const shell = await read("src/components/fusio/FusioAI.jsx");
  const bootstrap = await read("src/app/bootstrap.js");

  assert.match(shell, /Fusio AI/);
  assert.match(shell, /MicIcon/);
  assert.match(shell, /ChatIcon/);
  assert.doesNotMatch(shell, /Sparkles|BrainCircuit|BotIcon|Robot/);
  assert.match(bootstrap, /mountFusioAI/);
