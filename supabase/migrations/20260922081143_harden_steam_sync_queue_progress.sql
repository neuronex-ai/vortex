  ...
  loop
    begin
      ...
      if coalesce((v_result ->> 'ok')::boolean, false) then
        ...
        v_reason := coalesce(v_result ->> 'reason', 'Steam returned no usable game data');

        update public.steam_catalog_apps
        set
          detail_synced_at = now(),
          sync_attempts = sync_attempts + 1,
          last_sync_error = left(v_reason, 1000),
          is_active = case
            when v_reason = 'Not a game' then false
            else is_active
          end,
          last_seen_at = now()
        where steam_app_id = v_app_id;
      end if;
    exception when others then
      ...
      set
        sync_attempts = sync_attempts + 1,
        last_sync_error = left(sqlerrm, 1000),
        last_seen_at = now()
      where steam_app_id = v_app_id;
...
revoke all on function public.sync_steam_apps(bigint[])
